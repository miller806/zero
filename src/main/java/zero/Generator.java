...
import zero.model.Table;
import zero.util.FileHelper;
import zero.util.IOHelper;
import zero.util.StringTemplate;
...
public class Generator {
    private static final String WEBAPP_GENERATOR_INSERT_LOCATION = "webapp-zero-insert-location";
    ...
    private void generateTable(Table table) throws IOException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, TemplateException, URISyntaxException {
        System.out.println("***************************************************************");
        ...
        if ("0".equals(PropertiesProvider.getProperty("template.mode"))) {
            templateRootDir = new File(PropertiesProvider.getProperty("template.dir")).getAbsoluteFile();
        } else {
            templateRootDir = new File(ClassLoader.getSystemResource("template").toURI());
        }
